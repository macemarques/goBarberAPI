...
class AppointmentController {
  ...
  async store(req, res) {
    ...
    // Check if a self appointment has been made
    // se possivel refazer esse codigo
    const checkIfIsMe = await User.findOne({
      where: { id: provider_id, provider: true },
    });
    if (checkIfIsMe.id === req.userId) {
      return res
        .status(400)
    ...
    // Notify the provider that a new appointment has been made
